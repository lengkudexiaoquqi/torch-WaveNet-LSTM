...
def objective(trial):
    ...
    BATCH_SIZE = trial.suggest_int("batch_size",low=10,high=250)
    # BATCH_SIZE = 150
    ...
    INPUT_SIZE = Window_size * len(X_pos)

    dataset = Data_Split(x_pos=X_pos, y_pos=Y_pos, window_size=Window_size, predict_step=gc.STEP(), filter_size=gc.SSA_FILTER_COMPONENT(Y_pos))
    train_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=BATCH_SIZE, shuffle=True)
    dataset.X_test_scaler = torch.from_numpy(dataset.X_test_scaler)

    bp = BPNet(input_size=INPUT_SIZE, hidden_size=HIDDEN_SIZE, predict_step=1)
    ...
    logger.info("-------------------------------------------------")
    base_path = '../MVT_AQP/{}/bpnn/'.format(current_predict_column)
...
if __name__ == "__main__":
    ...
    for p in poses:
        ...
        filename = '../MVT_AQP/{0}/bpnn/{0}_{1}.log'.format(current_predict_column, current_time)
        ...
        study = optuna.create_study(study_name="bpnn", direction="minimize", sampler=sampler)
        ...
        df.to_csv("hyperopt_bpnn_{0}_{1}.csv".format(combine_name, indicators[Y_pos]))
        optuna.visualization.plot_slice(study, params=['hidden_size', 'learning_rate'])
        # 保存和恢复study
        joblib.dump(study, "study_bpnn_{0}_{1}.pkl".format(combine_name, df.columns[Y_pos]))
        # 恢复study
